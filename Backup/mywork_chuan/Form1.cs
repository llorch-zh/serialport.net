...
namespace mywork_chuan
{
    ...
    public partial class Form1 : Form
    {
       ...
       // bool isHEX = false;//16进制显示标志位
        ...
        {
            System.Threading.Thread.Sleep(10);//延时10ms等待接收完数据
            //代理，对数据进行处理
            this.Invoke((EventHandler)(delegate
            {
                int tmp = sp.BytesToRead;//读取总共有多少数据
                g = this.CreateGraphics();
                Pen p1 = new Pen(Color.Black);//定义黑色画笔，用于作图 
                for (int i = 0; i < tmp; i++)
                {
                    int a = Convert.ToInt32(sp.ReadByte());//逐字节读取
                    int b = Convert.ToInt32( a * 180/255);//换算Y值，当前画图区在200-380之间，共180像素
                    //richTextBox1.Text += a.ToString();//直接转string显示，不进行转码
                    //richTextBox1.Text += "\n";//数据之间用空格隔开                    
                    s.Write(a.ToString());
                    s.Write(' ');
                    //画图X轴范围在30-430之间，当图像画至430之后，需要将其置回初始值，图像清除
                    if (count !=10)//画心音波形
                    {
                        if (j >= 430)
                        {
                        ...
                        g.DrawLine(p1, j, last, j + (float)0.25, (float)380 - b);
                        last = 380 - b;//计算得当前Y坐标
                        j = j + (float)0.25;
                        num++;
                        label4.Text = num.ToString();
                    ...
                    else//心电波形，画图区在400-580之间，共180像素
                    {
                        count = 1;                   
                        if (j_h >= 430)
                        {
                        ...
                        g.DrawLine(p1, j_h, last_h, j_h + (float)0.25 * 9, (float)580 - b);
                        last_h = 580 - b;//计算得当前Y坐标
                        j_h = j_h + (float)0.25 * 9;
                        num++;
                        label4.Text = num.ToString();                     
